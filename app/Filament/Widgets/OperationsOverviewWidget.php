    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Peminjaman Aktif', $activeLoans)
                ->description($overdueLoans > 0 ? "{$overdueLoans} melewati jatuh tempo" : 'Semua tepat waktu')
            ...
            Stat::make('Koleksi Buku', $totalBooks)
                ->description("{$availableItems} dari {$totalItems} eksemplar tersedia")
                ->descriptionIcon(Heroicon::OutlinedBookOpen)
                ->color('info')
                ->icon(Heroicon::OutlinedBookOpen),

            Stat::make('Anggota Baru Bulan Ini', $newMembersThisMonth)
                ->description($pendingApproval > 0 ? "{$pendingApproval} menunggu persetujuan" : 'Semua sudah disetujui')
